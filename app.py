                ...
                if open_holdings:
                    ...
                    for idx, holding in enumerate(open_holdings):
                        with st.expander(f"📊 {holding['symbol']} - Qty: {holding['quantity']} | Entry: ${holding['entry_price']:.2f}", expanded=False):
                            ...
                            # Action tabs: Update Price, Edit Position, Close Position
                            action_tab1, action_tab2, action_tab3 = st.tabs(["🔄 Update Price", "✏️ Edit Position", "✅ Close Position"])
                            
                            with action_tab1:
                                st.markdown("**Update Current Market Price**")
                                ...
                                if st.button("🔄 Update Price", key=f"update_{holding['id']}", use_container_width=True):
                            ...
                            with action_tab2:
                                ...
                                with st.form(f"edit_form_{holding['id']}"):
                                    ...
                                    with col_preview2:
                                        new_unrealized = (edit_quantity * edit_current_price) - (edit_quantity * edit_entry_price)
